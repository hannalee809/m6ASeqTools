...
**Input size considerations:** m6AnetTools does not impose hard-coded limits on input file size. Performance is primarily constrained by available system memory and the number of m6A sites analyzed. For large datasets, we recommend processing data on systems with sufficient memory.
...
# File paths
rbp_file <- system.file("extdata", "RBP_POSTAR_human_subset.bed", package = "m6ASeqTools")
...
Use m6A_seq_part1() after running m6Anet and before any differential methylation or integration with gene expression analyses. The output of this function serves as the input for any further comparative analysis.
...
m6A_seq_part1() executes the following steps automatically:
...
m6A_seq_part1(
  ...
  output_directory = "m6ASeqTools_output",
  mod_ratio_df = "Naive_Rep1",
  probability = 0.9
)
```
...
The function m6a_seq_part2() performs downstream comparative analysis of m6A methylation between two biological groups and integrates these results with differential gene expression (DGE) data. This function is designed to be run after completing Part 1 of the m6ASeqTools pipeline and is optimized for studies without biological replicates.
...
Use m6a_seq_part2() once you have:
...
`m6a_seq_part2()` runs the following steps automatically:
...
m6a_seq_part2(
  vec_list = NULL,
  g1_df = weighted_mod_ratio_Naive,
  g2_df = weighted_mod_ratio_IGF2BP2_KD,
  group1_name = "Naive",
