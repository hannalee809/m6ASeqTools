...
> **Note:** By default, the function assumes the m6a and external bed columns are named chr, start, end, and strand, unless specified otherwise.

Both inputs are internally converted to `GenomicRanges` objects. Overlaps are computed with `findOverlaps()`, and all annotation metadata columns are retained. Strand matching can be required or ignored depending on the use case.
...
```{r annotate-m6A-example}
# Example of annotate_m6A() using bed file 
```
