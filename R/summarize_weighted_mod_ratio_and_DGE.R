...
summarize_weighted_mod_ratio_and_DGE <- function(df, group1_name="Group1", group2_name="Group2") {
  group_list <- list()
  ...
  # Loop through each condition to build the nested structure
  for (name in names(filters)) {
    ...
    # Transcript region summary (replace with correct column names or indices)
    region_summary <- tibble::tibble(
      Region = c("sum_sites_in_5utr", "sum_sites_in_cds", "sum_sites_in_3utr", "total_sites"),
      !!group1_name := c(
